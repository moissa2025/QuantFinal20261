...
export default function DeveloperPlayground() {
  const [method, setMethod] = useState("GET"    </DockablePanel>
  );
  const [endpoint, setEndpoint] = useState("/v1/markets"    </DockablePanel>
  );
  const [requestBody, setRequestBody] = useState("{\n  \n}"    </DockablePanel>
  );
  const [response, setResponse] = useState("// Response will appear here"    </DockablePanel>
  );
  ...
  function simulateRequest() {
    ...
    setResponse(JSON.stringify(mock[endpoint] || { message: "OK" }, null, 2)    </DockablePanel>
  );
  }
